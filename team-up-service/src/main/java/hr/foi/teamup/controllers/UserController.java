...
import hr.foi.teamup.repositories.UserRepository;
...
public class UserController {
    ...
    public ResponseEntity<User> login(@RequestBody Credentials credentials) {
        ...
        User found = this.personRepository.findByUsername(username);
    ...
    public ResponseEntity<User> signup(@RequestBody User person) {
        Logger.getLogger("PersonController.java").log(Level.INFO,
                "POST on /person/signup -- " + person.toString());
        
        User signed = this.personRepository.save(person);
        if(signed != null) {
            Logger.getLogger("PersonController.java").log(Level.INFO,
    ...
    public ResponseEntity<User> retrieveById(@RequestParam long id) {
        Logger.getLogger("PersonController.java").log(Level.INFO,
                "GET on /person/" + id + " -- ");
        User found = this.personRepository.findById(id);
        if(found != null) {
            Logger.getLogger("PersonController.java").log(Level.INFO,
